...
def cross_entropy_loss(probs, target_index):
    ...
    probs = D_array_for_2D(probs)
    batch_size = probs.shape[0]
    probs = -np.log(probs)
    return np.sum(probs[(np.arange(batch_size), target_index)]) / batch_size
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for batch_index in batches_indices:
                batch = X[batch_index, :]
                target_index = y[batch_index]
                loss, dW = linear_softmax(batch, self.W, target_index)
